...
违约业绩接口测试用例
"""
case_description = u"委托合同提交终止结算，选择正退。验证没有生成违约业绩"
...
def break_case_003():
    ...
    contractEndRequest.house_contract_end(contract_num,"正退","2018-04-16")
    for i in range(10):
        ...
        if result:
            ...
            sleep(2)
    if not result:
        return u"用例编号：break_case_003测试通过！ 用例描述：" + case_description
    else:
        return u"用例编号：break_case_003测试不通过！ 用例描述：" + case_description + u"测试结果：" + result

contractRequest.host_set("mock")
contractRequest.get_cookie()
print break_case_003()