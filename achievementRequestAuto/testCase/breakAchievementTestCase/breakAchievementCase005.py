...
违约业绩接口测试用例
"""
case_description = u"出租提交终止结算，选择转租。验证生成一条违约业绩"
...
def break_case_005():
    ...
    contractEndRequest.apartment_contract_end(contract_nums,"2018-12-30","转租")
    for i in range(10):
        ...
        if result:
            ...
            sleep(5)
    if result:
        return u"用例编号：break_case_005测试通过！ 用例描述：" + case_description
    else:
        return u"用例编号：break_case_005测试不通过！ 用例描述：" + case_description + u"测试结果：" + str(result)+ u"测试数据。出租合同号："+contract_nums

contractRequest.host_set("mock")
contractRequest.get_cookie()
print break_case_005()