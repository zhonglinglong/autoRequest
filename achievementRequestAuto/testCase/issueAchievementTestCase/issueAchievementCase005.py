...
出单业绩接口测试用例
"""
case_description = u"品牌整租,出租合同委托合同均已复审,生成一条生效的业绩"
...
def issue_case_005():
    ...
    for i in range(10):
        ...
        if result != "Y":
            ...
            break
    if result == "Y":
        return u"用例编号：issue_case_005测试通过！ 用例描述：" + case_description
    else:
        return u"用例编号：issue_case_005测试不通过！ 用例描述：" + case_description + u"测试结果：" + result

contractRequest.host_set("mock")
contractRequest.get_cookie()
print issue_case_005()
