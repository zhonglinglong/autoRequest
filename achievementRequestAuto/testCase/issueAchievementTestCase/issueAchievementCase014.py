...
"""
case_description = u"品牌整租,前承租合同终止结算类型是收房，已复审。首付款完成。验证生成一条已生效的业绩"
...
def issue_case_014():
    ...
    for i in range(10):
        ...
        if result != "Y":
            ...
            break
    if result == "Y":
        return u"用例编号：issue_case_014测试通过！ 用例描述：" + case_description
    else:
        return u"用例编号：issue_case_014测试不通过！ 用例描述：" + case_description + u"测试结果：" + result

contractRequest.host_set("mock")
contractRequest.get_cookie()
print issue_case_014()