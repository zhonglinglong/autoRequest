...
"""
case_description = u"品牌整租,前承租合同终止结算类型是收房，且已通过复审,节点变成Y"
...
def issue_case_010():
    ...
    if result[u"前承租合同终止结算已通过复审（针对转租、退租、换租和收房）"] == "Y":
        return u"用例编号：issue_case_010测试通过！ 用例描述：" + case_description
    else:
        return u"用例编号：issue_case_010测试不通过！ 用例描述：" + case_description + u"测试结果：" + result

contractRequest.host_set("mock")
contractRequest.get_cookie()
print issue_case_010()