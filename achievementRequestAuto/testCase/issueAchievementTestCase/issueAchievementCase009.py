...
"""
case_description = u"品牌整租,前承租合同终止结算类型是换租，且已通过复审,节点变成Y"
...
def issue_case_009():
    ...
    contractEndRequest.apartment_contract_end(contract_nums,"2019-01-31","换租","2018-04-01")
    sleep(2)
    ...
    if result[u"前承租合同终止结算已通过复审（针对转租、退租、换租和收房）"] == "Y":
        return u"用例编号：issue_case_009测试通过！ 用例描述：" + case_description
    else:
        return u"用例编号：issue_case_009测试不通过！ 用例描述：" + case_description + u"测试结果：" + result

contractRequest.host_set("mock")
contractRequest.get_cookie()
print issue_case_009()