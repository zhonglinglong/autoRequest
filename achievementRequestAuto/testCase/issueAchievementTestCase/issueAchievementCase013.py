...
"""
case_description = u"品牌整租,前承租合同终止结算类型是收房，未复审。首付款完成。验证生成一条未生效的业绩"
...
def issue_case_013():
    ...
    if result == "N":
        return u"用例编号：issue_case_013测试通过！ 用例描述：" + case_description
    else:
        return u"用例编号：issue_case_013测试不通过！ 用例描述：" + case_description + u"测试结果：" + result

contractRequest.host_set("mock")
contractRequest.get_cookie()
print issue_case_013()